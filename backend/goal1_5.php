...
$strSQL  = "select * from (
	...
	case WHEN  ROUND((IPDDEATH.IPDDISCHARGE / x.ipddischarge) * 100,2) < 28 then 'text-success' else ' text-danger '  end as textcolor,
	case WHEN  ROUND ((IPDDEATH.IPDDISCHARGE / x.ipddischarge) * 100,2) < 28 then 'border-success ' else 'border-danger  '  end as bordercolor,
	case WHEN  ROUND ((IPDDEATH.IPDDISCHARGE / x.ipddischarge) * 100,2) < 28 then ' bg-success' else ' bg-danger '  end as bgcolor,
	oldyear.dataperson as datapersonold,
	case when  oldyear.dataperson  < 28 then  'text-success' else ' text-danger '  end as updowncolor,
	case when  oldyear.dataperson  < 28 then  'arrow-down' else 'arrow-up'  end as updownicon
